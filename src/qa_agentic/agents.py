...
def planner_node(ctx: Context) -> PlannerOutput:
    ...
    def _invoke_planner(extra_answers: dict | None = None) -> PlannerOutput:
        sys_prompt = SystemMessage(content=(
            ...
            "{name, kind, target, metric, columns, where, comparator, threshold{source,value,units}, evidence, on_sql_index}. "
            "If acceptance criteria lack critical details (thresholds, dimensions), set ready=false and list clarifications. "
            "Do NOT produce code."
        ))
        payload = {
            "story": ctx.story,
        ...
        user_prompt = HumanMessage(content=json.dumps(payload))
        return llm.with_structured_output(PlannerOutput).invoke([sys_prompt, user_prompt])

    plan = _invoke_planner()

    # HITL round if needed
...
def codegen_node(ctx: Context, plan: PlannerOutput, history: List[Dict[str, Any]] | None = None, prev_error: str | None = None) -> Dict[str, Any]:
    ...
    user_prompt = HumanMessage(content=json.dumps(payload))
    out = llm.with_structured_output(CodegenOutput).invoke([sys_prompt, user_prompt])
    return {"sql": out.sql}
