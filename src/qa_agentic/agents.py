...
class Threshold(BaseModel):
    source: str = Field(..., description="Origin of the threshold value (e.g., acceptance criteria, HITL, auto, default)")
    value: Any | None = Field(None, description="Threshold value; can be a number, range, enum list, or None")
    units: Optional[str] = Field(None, description="Units for the threshold (e.g., percent, count)")

class TestCheck(BaseModel):
    name: str = Field(..., description="Name of the test check")
    kind: str = Field(..., description="Type of check")
    target: str = Field("sql_result")
    metric: str = Field(..., description="Metric to evaluate: row_count, null_pct, accepted_values, range, join_coverage, custom_sql_bool,etc")
    columns: List[str] = Field(default_factory=list, description="Columns involved in the check (for column metrics)")
    where: Optional[str] = Field(None, description="Optional WHERE clause for filtering")
    comparator: str = Field(">=", description="Comparator for threshold evaluation (e.g., >=, <=, ==)")
    threshold: Threshold = Field(default_factory=lambda: Threshold(source="default", value=None), description="Threshold specification for the check")
    evidence: str = Field("counts", description="Type of evidence used for the check (e.g., counts)")
    on_sql_index: int = Field(0, description="Index of the generated SQL result this check applies to")

class PlannerOutput(BaseModel):
    tools_needed: List[str] = Field(default_factory=list, description="List of tool names required for the plan")
    checks: List[TestCheck] = Field(default_factory=list, description="List of test checks to be performed")
    clarifications: List[str] = Field(default_factory=list, description="List of clarifications required from the user")
    ready: bool = Field(True, description="Indicates if the plan is ready or if HITL is required to proceed")

class CodegenOutput(BaseModel):
    sql: List[str] = Field(default_factory=list, description="List of generated SQL statements")
...
def build_llm(settings: dict):
    ...
    return init_chat_model(model, model_provider="ollama", base_url=base_url,reasoning=True,temperature=0)
...
def _structured_call(llm, schema_model: Type[BaseModel], system_text: str, user_payload: dict):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "{system_text}\n\nYou MUST return only valid JSON matching the schema below and make sure all fields are provided but do not hallucinate or assume: \n{format_instructions}"),
        ("human", "{payload_json}")
    ])
...
def planner_node(ctx: Context) -> PlannerOutput:
    ...
    def _invoke_planner(extra_answers: dict | None = None) -> PlannerOutput:
        system_text = (
            ...
            "If acceptance criteria lack critical details or need clarification (thresholds, dimensions),etc. "
            "then, set ready=False and list clarifications required from user. Do NOT produce code."
        )
...
def codegen_node(ctx: Context, plan: PlannerOutput, history: List[Dict[str, Any]] | None = None, prev_error: str | None = None) -> Dict[str, Any]:
    ...
    system_text = (
        ...
        "Only SELECT/WITH . If a prior attempt failed, use prior SQL and error messages to repair and generate the entire SQL again. "
    )
    payload = {
        "story": ctx.story,
        "schema": ctx.schema,
